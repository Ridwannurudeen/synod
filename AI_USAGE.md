...
- **Google Gemini API** (`gemini-2.5-flash`) — used at runtime as Settler C. The live network is **two providers across four models**, not "three providers" — Sonnet and Haiku are both Anthropic.
...
3. **Heterogeneity scope decision.** When the AI proposed a 5-vendor heterogeneous settler network, human cut the live demo scope first, then restored provider adapters once the AXL/on-chain core was stable. The shipped live network runs **two providers** across **four model variants**: Anthropic `claude-sonnet-4-6` (Settler A), Anthropic `claude-haiku-4-5` (Settler B), Google `gemini-2.5-flash` (Settler C), and Anthropic `claude-opus-4-7` (Settler D, on a separate VPS in Toronto). Heterogeneity is real on the model/vendor axis. Per-role specialization (analyst / skeptic / synthesizer) was discussed but not implemented — all settlers use the same SYSTEM_PROMPT in v1. OpenAI is supported by the provider abstraction but kept off the live demo for cost.
