...
3. **Heterogeneity scope decision.** When the AI proposed a 5-vendor heterogeneous settler network, human cut scope to single-vendor (Anthropic only) for the v1 build, with multi-vendor as the explicit roadmap item — a deliberate cost/scope tradeoff the AI didn't make on its own.
