...
- **Anthropic Claude API** — used at runtime as settler models. Settler A runs `claude-sonnet-4-6`, Settler B runs `claude-haiku-4-5`. Sonnet was chosen over Opus for cost efficiency (5× cheaper, indistinguishable quality on JSON-structured outputs).
- **Google Gemini API** (`gemini-2.5-flash`) — used at runtime as Settler C, demonstrating provider heterogeneity in the swarm (not all settlers are Anthropic).
...
3. **Heterogeneity scope decision.** When the AI proposed a 5-vendor heterogeneous settler network, human cut the live demo scope first, then restored provider adapters once the AXL/on-chain core was stable. The shipped live network runs three providers: Anthropic Sonnet 4.6 (Settler A), Anthropic Haiku 4.5 (Settler B), and Google Gemini 2.5 Flash (Settler C). Settler D on Toronto (Servarica VPS) demonstrates cross-machine AXL meshing. OpenAI is supported by the provider abstraction but kept off the live demo for cost.
